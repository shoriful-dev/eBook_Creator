...
const ChapterEditorTab = ({
  ...
  return <div className={`${isFullscreen ? 'fixed inset-0 z-50 bg-white' : 'flex-1'} flex flex-col`}>
    {/* Header */}
    <div className="borber-b border-gray-100 px-4 bg-white">
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-0 justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden">
              <button onClick={() => setIsPreviewMode(false)} className={`px-3 py2 text-sm font-medium transition-colors ${!isPreviewMode ? 'text-violet-700 bg-violet-50 border-r border-violet-200' : 'text-gray-600 hover:bg-gray-50'}`}>
                Edit
              </button>
              <button onClick={() => setIsPreviewMode(true)} className={`px-3 py-2 text-sm font-medium transition-colors ${isPreviewMode ? 'text-violet-700 bg-violet-50' : 'text-gray-600 hover:bg-gray-50'}`}>
                Preview
              </button>
            </div>

            <button onClick={() => setIsFullscreen(!isFullscreen)} className="p-2 rounded-lg transition-colors text-gray-600 hover:bg-gray-100" title='Toggle Fullscsreen'>
                ...
                <div className="h-full border border-gray-200 rounded-lg overflow-y-auto">
                  ...
                  <div className="p-8">
                    ...
                    <div
                     ...
                     dangerouslySetInnerHTML={{__html: currentChapter.content ? formatMarkdown(currentChapter.content) : '<p class="text-gray-400 italic">No Content yet. Start writing to see the preview.</p>'}}
                     />
                  </div>
                </div>
                ) : (
