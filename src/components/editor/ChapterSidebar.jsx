...
const SortableItem = ({
  ...
  return (
    <div ref={setNodeRef} style={style} className='group flex items-center bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 relative overflow-hidden'>
      <button
        className={`flex-1 flex items-center p-3 text-sm rounded-l-lg text-left transition-colors ${selectedChapterIndex === index ? 'bg-violet-50/50 text-violet-800 font-semibold' : 'hover:bg-slate-100 text-slate-600'}`}
